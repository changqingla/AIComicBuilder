...
import {
  GET as getBindings,
  PUT as putBinding,
} from "@/app/api/projects/[id]/agent-bindings/route";
...
beforeAll(() => {
  ...
  for (const suffix of ["a", "b"]) {
    db.insert(projects)
      .values({ id: `p-${suffix}`, userId: `user-${suffix}`, title: suffix })
      .run();
    db.insert(episodes)
      .values({
        id: `ep-${suffix}`,
...
test.each([
  ...
  expect(
    db.$client.prepare("SELECT idea FROM episodes WHERE id = 'ep-b'").get(),
  ).toEqual({ idea: "" });
});

test("unknown generation actions are rejected instead of enqueued", async () => {
  expect(
    (await generate(request({ action: "unknown" }), routeParams)).status,
  ).toBe(400);
  expect(
    db.$client
...
test("binding reads and writes require ownership of the project and agent", async () => {
  ...
  expect(
    (
      await putBinding(
        request(
          { category: "script_generate", agentId: "agent-b" },
          "user-a",
          "PUT",
        ),
        routeParams,
  ...
  expect(
    (
      await putBinding(
        request(
          { category: "script_generate", agentId: "agent-a" },
          "user-a",
          "PUT",
        ),
        routeParams,
...
test("missing model or invalid generated characters preserve existing links", async () => {
  ...
  expect(
    db.$client
      .prepare(
        "SELECT character_id FROM episode_characters WHERE episode_id = 'ep-a'",
      )
      .all(),
  ).toEqual([{ character_id: "char-a" }]);
});

test("a failed character replacement rolls back changes to characters and links", () => {
  db.$client.exec(
    "CREATE TRIGGER reject_character_link BEFORE INSERT ON episode_characters BEGIN SELECT RAISE(ABORT, 'test failure'); END",
  );
  try {
    expect(() =>
      saveExtractedCharacters(
        "p-a",
        "ep-a",
        characterExtractionSchema.parse({
    ...
    expect(
      db.$client
        .prepare("SELECT description FROM characters WHERE id = 'char-a'")
        .get(),
    ).toEqual({ description: "Existing description" });
    expect(
      db.$client
        .prepare(
          "SELECT character_id FROM episode_characters WHERE episode_id = 'ep-a'",
...
test("file reads reject adjacent directories and symlinks outside uploads", async () => {
  ...
  for (const segments of [["../uploads-private/hidden.txt"], ["link.txt"]]) {
    expect(
      (
        await getUpload(request(null, "", "GET"), {
  ...
  expect(
    await (
      await getUpload(request(null, "", "GET"), {
        params: Promise.resolve({ path: ["valid.txt"] }),
      })
    ).text(),
  ).toBe("visible");
});
