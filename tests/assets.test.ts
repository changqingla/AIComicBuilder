...
test("concurrent results preserve history and have one active version", async () => {
  ...
  await expect(activateAssetVersion("s", "first_frame", 0, 99)).rejects.toThrow(
    "not found",
  );
  expect((await getActiveAsset("s", "first_frame"))?.fileUrl).toBe(
    "image-1.png",
...
test("regeneration inherits metadata from the selected historical version", async () => {
  insertAssetVersion({
    shotId: "s",
    type: "reference",
    prompt: "A",
  ...
  await activateAssetVersion("s", "reference", 0, 1);
  const result = insertAssetVersion({
    shotId: "s",
    type: "reference",
    prompt: "A again",
    fileUrl: "new.png",
