...
export default function ImportPage({
  ...
  const [stepStatus, setStepStatus] = useState<
    Record<Step, "idle" | "running" | "done" | "error">
  >({
    1: "idle",
    2: "idle",
    3: "idle",
    ...
    async function loadLogs() {
      try {
        ...
        if (data.length > 0) {
          ...
          const doneSteps = data
            .filter((l: LogEntry) => l.status === "done")
            .map((l: LogEntry) => l.step);
  ...
  const addLog = useCallback(
    (step: Step, status: LogEntry["status"], message: string) => {
      setLogs((prev) => [
        ...prev,
        {
  ...
  async function startPipeline() {
    ...
    try {
      const res = await apiFetch(
        `/api/projects/${projectId}/import/characters`,
        {
          method: "POST",
      ...
      const mainCount = data.characters.filter(
        (c: ExtractedCharacter) => c.scope === "main",
      ).length;
  ...
  async function runGenerate() {
    ...
    try {
      ...
      addLog(
        4,
        "done",
        `导入完成！创建了 ${data.characterCount} 个角色和 ${data.episodes.length} 集`,
      );
  ...
  function retryStep() {
    const failedStep = ([1, 2, 3, 4] as Step[]).find(
      (s) => stepStatus[s] === "error",
    );
  ...
  const showCharReview =
    stepStatus[2] === "done" && stepStatus[3] === "idle" && !historyMode;
  // Show episodes review after step 3 done + step 4 idle
  const showEpReview =
    stepStatus[3] === "done" && stepStatus[4] === "idle" && !historyMode;
          ...
          {STEPS.map(({ num, icon: Icon, label }) => {
            ...
            return (
              <button
                ...
                onClick={() =>
                  isClickable && setSelectedStep(isSelected ? null : num)
                }
        ...
        {(currentStep > 0 || historyMode) &&
          ...
          (() => {
            ...
            const stepDoneLog = selectedStep
              ...
              : null;
            const meta = stepDoneLog?.metadata as Record<
              string,
              unknown
            > | null;
            const metaCharacters = meta?.characters as
            ...
            const step2DoneLog =
              selectedStep === 3
                ? logs.find(
                    (l) => l.step === 2 && l.status === "done" && l.metadata,
                  )
                : null;
            const step2Meta = step2DoneLog?.metadata as Record<
              string,
              unknown
            > | null;
            const step2Characters = step2Meta?.characters as
              ExtractedCharacter[] | undefined;
                      ...
                      <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-3">
                        {metaCharacters.map((char, idx) => (
                          ...
                          >
                            ...
                            <span
                              className={`absolute right-3 top-3 rounded-[8px] px-2 py-0.5 text-[9px] font-bold tracking-wide ${
                                char.scope === "main"
                        ...
                        {metaEpisodes.map((ep, idx) => (
                          ...
                          >
                            ...
                            {ep.characters && ep.characters.length > 0 && (
                              <div className="mt-2 flex flex-wrap gap-1">
                                {ep.characters.map((name) => {
                                  const isMain = step2Characters?.some(
                                    (c) =>
                                      c.name === name && c.scope === "main",
                                  );
                                  return (
                                    <span
                                      key={name}
