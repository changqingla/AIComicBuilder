...
import { handleScriptOutlineAction, handleScriptGenerate, handleScriptParseStream } from "@/lib/generation/scripts";
import { handleCharacterExtract, handleSingleCharacterImage, handleBatchCharacterImage } from "@/lib/generation/characters";
import { handleShotSplitStream, handleSingleShotRewrite } from "@/lib/generation/storyboards";
import { handleBatchFrameGenerate, handleSingleFrameGenerate } from "@/lib/generation/frames";
import { handleGenerateKeyframePrompts } from "@/lib/generation/keyframe-prompts";
import { handleSingleVideoGenerate, handleBatchVideoGenerate } from "@/lib/generation/videos";
import { handleSingleReferenceVideo, handleBatchReferenceVideo } from "@/lib/generation/reference-videos";
import { handleSingleSceneFrame, handleBatchSceneFrame, handleBatchRefImageGenerate, handleSingleRefImageGenerate, handleSingleShotRefImageGenerateAll } from "@/lib/generation/reference-images";
import { handleGenerateRefPrompts } from "@/lib/generation/reference-prompts";
import { handleSingleVideoPrompt, handleBatchVideoPrompt } from "@/lib/generation/video-prompts";
import { handleVideoAssembleSync } from "@/lib/generation/assembly";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!hasGenerationAccess(projectId, parsed.data)) return NextResponse.json({ error: "Resource not found" }, { status: 404 });
  try {
    const result = await handlers[parsed.data.action]({ ...parsed.data, projectId, userId: project.userId });
    return result instanceof ReadableStream
      ? new Response(result, { headers: { "Content-Type": "text/plain; charset=utf-8" } })
