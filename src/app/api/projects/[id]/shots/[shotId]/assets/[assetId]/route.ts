...
const patchSchema = assetEditSchema
  .omit({ id: true, sequenceInType: true })
  .partial();
...
export async function PATCH(
  ...
  {
    params,
  }: { params: Promise<{ id: string; shotId: string; assetId: string }> },
) {
  ...
  const target = db
    .select()
    .from(shotAssets)
    .where(and(eq(shotAssets.id, assetId), eq(shotAssets.shotId, shotId)))
    .get();
  if (!target)
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  if (!target.isActive)
  ...
  if (!parsed.success)
    return NextResponse.json(
      { error: "Invalid asset changes" },
      { status: 400 },
    );
  const { characters, ...fields } = parsed.data;
  db.update(shotAssets)
    .set({
      ...fields,
      updatedAt: new Date(),
      ...(characters !== undefined
        ? {
            characters: characters === null ? null : JSON.stringify(characters),
          }
        : {}),
    })
