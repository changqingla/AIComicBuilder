...
}).refine(({ type, sequenceInType }) => type === "reference" || sequenceInType === 0);
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; shotId: string }> }) {
  ...
  if (!(file instanceof File) || !parsed.success || !["image/png", "image/jpeg", "image/webp"].includes(file.type)) {
    return NextResponse.json({ error: "Invalid image or asset position" }, { status: 400 });
  }
  ...
  const asset = insertAssetVersion({ shotId, type, sequenceInType, fileUrl,
    prompt: previous?.prompt ?? "", status: "completed" });
