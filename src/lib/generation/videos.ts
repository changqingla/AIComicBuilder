...
export async function handleSingleVideoGenerate(input: GenerationInput) {
  ...
  if (!firstFrame || !lastFrame)
    throw new ApiError(400, "Shot frames not generated yet");
  ...
  const videoSlots = await resolveSlotContents("video_generate", {
    userId,
    projectId,
  });
  ...
  try {
    ...
    const videoPrompt =
      ...
      buildVideoPrompt({
        ...
        startFrameDesc: selectAsset(assets, "first_frame")?.prompt ?? undefined,
        endFrameDesc: selectAsset(assets, "last_frame")?.prompt ?? undefined,
