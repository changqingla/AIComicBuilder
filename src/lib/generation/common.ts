...
import { agentBindings,agents,characters,episodeCharacters,storyboardVersions } from "@/lib/db/schema";
...
export async function getVersionedUploadDir(versionId: string | null | undefined): Promise<string> {
  ...
  return path.join(process.env.UPLOAD_DIR || "./uploads", "projects", version.projectId, versionId);
}
