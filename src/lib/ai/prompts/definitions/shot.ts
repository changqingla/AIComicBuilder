...
import { resolve, slot, type PromptDefinition } from "../definition";
...
4. **每一个具体物件/道具**：剧本提到的带名字的道具、服饰细节、环境物件（"那只磨损的皮质公文包"、"墙上泛黄的全家福"、"半杯冷掉的咖啡"）必须出现在 sceneDescription 中的至少一处。
...
✅ 正确的展开：拆成 1-2 个镜头，motionScript 里明确"推开吱呀作响的木门→雨帘中愣住→右手探入风衣口袋摸到那封未寄出的信→指尖停顿片刻→嘴角牵起一丝自嘲的弧度"，sceneDescription 里写"深夜雨巷，远处飘来馄饨摊老人沙哑的吆喝声"，信作为关键道具出现在 sceneDescription 的构图里。
...
- 不要包含角色的具体动作或姿态——那些放在 motionScript 中（但可以写角色已经在场的事实）
...
const SHOT_SPLIT_LANGUAGE_RULES = `【关键语言规则】所有文本字段（sceneDescription、motionScript、videoScript、dialogues.text、dialogues.character）必须使用与剧本相同的语言。如果剧本是中文，所有字段都用中文。只有"cameraDirection"使用英文（技术术语）。
...
export const shotSplitDef: PromptDefinition = {
  ...
  buildFullPrompt(sc, params) {
    ...
    const replaceDuration = (text: string) =>
      text
