"use client";
import { useShotEditor, type ShotCardProps } from "@/hooks/use-shot-editor";
...
function StepRow({
  ...
  const open = openOverride ?? (defaultOpen || isNext);
        ...
        <div className="flex gap-1">
          {(generationMode === "reference"
            ...
            return (
              <div
                key={i}
                className="h-8 w-11 flex-shrink-0 overflow-hidden rounded-md border border-[--border-subtle] bg-[--surface]"
              >
                {src ? (
                  isVid ? (
                    <video
        ...
        <div className="order-last min-w-0 basis-full md:order-none md:flex-1">
          ...
          <div className="mt-1 flex items-center gap-2 flex-wrap">
            <div className="flex flex-wrap items-center gap-2 text-xs">
              ...
              <select
                ...
                onChange={(e) => {
                  patchShot({ transitionIn: e.target.value });
                  onUpdate();
                }}
                onClick={(e) => e.stopPropagation()}
              ...
              <select
                ...
                onChange={(e) => {
                  patchShot({ transitionOut: e.target.value });
                  onUpdate();
                }}
                onClick={(e) => e.stopPropagation()}
                className="h-7 rounded border border-[--border-subtle] bg-white px-2 text-xs outline-none focus:border-primary/50"
          ...
          {(soundDesign || musicCue) && (
            <div className="mt-1 flex items-center gap-2 flex-wrap text-xs text-[--text-muted]">
              {soundDesign && (
                <span>
                  {t("shot.sfx")}: {soundDesign}
                </span>
              )}
              {musicCue && (
                <span>
                  {t("shot.music")}: {musicCue}
                </span>
              )}
            </div>
          )}
      ...
      <div className="space-y-2 border-t border-[--border-subtle] px-4 pb-3 pt-3">
            ...
            <div>
              <div className="mb-1 flex items-center gap-1">
                ...
                <AiOptimizeButton
                  ...
                  onOptimized={(v) => {
                    setEditMotionScript(v);
                    patchShot({ motionScript: v });
                  }}
        ...
        >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-2">
                  {parsedRefImages.map((ref, refIdx) => (
                    ...
                    >
                      ...
                      <div
                        className={`relative bg-[--surface] ${ref.fileUrl ? "aspect-video" : "h-20"}`}
                      >
                        {ref.fileUrl ? (
                          <img
                            alt={String(
                              ref.meta?.sceneName || t("shot.sceneRefFrame"),
                            )}
                            src={uploadUrl(ref.fileUrl)}
                            className="w-full h-full object-cover cursor-pointer hover:opacity-80 transition-opacity"
                            onClick={() =>
                              setPreviewSrc(uploadUrl(ref.fileUrl!))
                            }
                          />
                        ) : (
                        ...
                        {(() => {
                          const history = assetHistory(ref).map(
                            (asset) => asset.fileUrl,
                          );
                          if (history.length < 2) return null;
                          const currentIdx = ref.fileUrl
                            ? history.indexOf(ref.fileUrl)
                            : -1;
                          return (
                            <>
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleSwitchRefImageVersion(ref.id, "prev");
                                }}
                              ...
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleSwitchRefImageVersion(ref.id, "next");
                                }}
                      ...
                      <div className="border-t border-[--border-subtle] px-2 py-1 bg-primary/5">
                        <span className="inline-flex items-center rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-medium text-primary">
                          {ref.meta?.sceneName ||
                            `${t("shot.scene")} ${refIdx + 1}`}
                        </span>
                      </div>
                      {/* Editable prompt with auto-save */}
                      <div className="border-t border-[--border-subtle]">
                        <div className="flex items-center gap-1 px-2 pt-1">
                          <p className="text-[9px] font-semibold uppercase tracking-[0.1em] text-violet-500">
                            {t("shot.refImagePrompt")}
                          </p>
                          <AiOptimizeButton
                            value={getRefPromptValue(ref.id, ref.prompt)}
                            onOptimized={(v) => {
                              setLocalRefPrompts((prev) => ({
                                ...prev,
                                [ref.id]: v,
                              }));
                        ...
                        <textarea
                          value={getRefPromptValue(ref.id, ref.prompt)}
                          onChange={(e) =>
                            handleRefPromptChange(ref.id, e.target.value)
                          }
                          onBlur={(e) =>
                            handleUpdateRefPrompt(ref.id, e.target.value)
                          }
                      ...
                      <div className="flex items-center gap-1 flex-wrap border-t border-[--border-subtle] px-2 py-1.5">
                        <span className="text-[9px] text-[--text-muted] shrink-0">
                          {t("shot.refChars") || "Chars"}:
                        </span>
                        {projectCharacters.map((char) => {
                          const isSelected = ref.characters?.includes(
                            char.name,
                          );
                          return (
                            <button
            ...
            <div className="mb-2.5 space-y-2">
              {(() => {
                ...
                const hasFirstPrompt =
                  !!firstFrameItem?.prompt || !!editStartFrame;
                ...
                <div className="grid grid-cols-2 gap-2">
                  {frameAssets.map((asset, i) => {
                    ...
                    const frameHistoryIds = frameItem
                      ? historyIds(frameItem)
                      : [];

                    const frameCurrentIdx = frameItem
                      ? frameHistoryIds.indexOf(frameItem.id)
                      : -1;
                    return (
                      <div
                        key={i}
                      ...
                      >
                        ...
                        <div
                          ...
                          onClick={() =>
                            asset.src &&
                            !isUploading &&
                            setPreviewSrc(uploadUrl(asset.src))
                          }
                        >
                          {isUploading ? (
                            <div className="flex h-full items-center justify-center">
                          ...
                          <div className="ml-auto">
                            <AiOptimizeButton
                              ...
                              onOptimized={(v) => {
                                setEditValue(v);
                                saveKeyframePrompt(
                                  isStart ? "first_frame" : "last_frame",
                                  v,
                                );
                              }}
                              fieldLabel={dbField}
                        ...
                        {(() => {
                          ...
                          return (
                            <div className="flex items-center gap-1 flex-wrap border-t border-[--border-subtle] px-2 py-1.5">
                              ...
                              {projectCharacters.map((char) => {
                                const isSelected = currentChars.includes(
                                  char.name,
                                );
                                return (
                                  <button
          ...
          >
            ...
            {generatingFrames || generatingSceneFrame || batchGeneratingFrames
              ...
              : generationMode === "reference"
                ? hasRefImages
                  ? t("shot.regenerateRefImages")
                  : t("shot.generateRefImages")
                : hasFrame
        ...
        >
            ...
            <div className="mb-2">
              <div className="mb-1 flex items-center gap-1">
                <AiOptimizeButton
                  ...
                  onOptimized={(v) => {
                    setEditVideoPrompt(v);
                    patchShot({ videoPrompt: v });
                  }}
          ...
          <Button
            ...
            disabled={
              generatingPrompt || batchGeneratingVideoPrompts || !hasFrame
            }
          >
            ...
            (() => {
              const videoTypeKey =
                generationMode === "reference"
                  ? "reference_video"
                  : "keyframe_video";
              const videoItem = allRefItems.find(
                (r) => r.type === videoTypeKey,
              );
              const videoHistoryIds = videoItem ? historyIds(videoItem) : [];
              const videoCurrentIdx = videoItem
                ? videoHistoryIds.indexOf(videoItem.id)
                : -1;
              return (
                <div
